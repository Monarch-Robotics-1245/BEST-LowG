...
def robot(leftMotor, rightMotor, motorOne, motorTwo, servoOne, servoTwo, servoThree, servoFour, digitalOne, digitalTwo, digitalThree, digitalFour, digitalFive, digitalSix, digitalSeven, digitalEight, digitalNine, driveFwd, driveSide, dbgUp, dbgDown, previousDbgDown, previousDbgUp, grip, release, previousGrip, previousRelease, towerAxis, lastPosition, lastPositionSet, turretAxis, analogOne, wristDown, wristUp, previousWristDown, previousWristUp, turretUp, previousTurretUp, previousTurretDown, turretDown, modeToggle, previousModeToggle, state, targetPosition, autoAButton, previousAutoAButton, autoBButton, previousAutoBButton, autoID, reverseButton, previousReverseButton, speedButton, previousSpeedButton, speedMultiplier, direction, autoStage):
    ...
    if state == 0:
        commands.drivetrain.drive(driveFwd, driveSide, reverseButton, previousReverseButton, direction, speedMultiplier, speedButton, previousSpeedButton)
        commands.drawBridge.runDrawBridge(dbgUp, dbgDown, previousDbgUp, previousDbgDown)
        commands.gripper.gripper(grip, release, previousGrip, previousRelease)
        commands.tower.moveTower(towerAxis, digitalOne, digitalTwo, digitalThree, digitalFour, digitalFive,
                                 lastPosition, lastPositionSet)
        commands.turret.rotateTurret(turretAxis, analogOne)
        commands.wrist.moveWrist(wristDown, wristUp, previousWristDown, previousWristUp)
    elif state == 1:
        autos.auto.auto(autoStage, digitalSix, digitalSeven, analogOne, lastPosition, lastPositionSet, digitalOne, digitalTwo, digitalThree, digitalFour, digitalFive, autoID)
    elif state == 2:
        commands.drivetrain.drive(driveFwd, driveSide, leftMotor, rightMotor)
        commands.drawBridge.runDrawBridge(dbgUp, dbgDown, previousDbgUp, previousDbgDown)
        commands.gripper.gripper(grip, release, previousGrip, previousRelease)
        commands.turret.rotateTurret(turretAxis, analogOne)
        commands.wrist.moveWrist(wristDown, wristUp, previousWristDown, previousWristUp, servoTwo)
        ...
        if targetPosition == 0:
            commands.tower.moveTower(towerAxis, digitalOne, digitalTwo, digitalThree, digitalFour,
                                     digitalFive, lastPosition, lastPositionSet)
        else:
            subsystems.tower.setTowerPosition(digitalOne, digitalTwo, digitalThree, digitalFour, digitalFive,
